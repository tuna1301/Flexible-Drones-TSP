...
for i in range(0,iter_proposed_model):
    ...
    df.to_csv ('export_distance_2drones.csv', index = False, header=True)             
    df = pd.read_csv('export_distance_2drones.csv')                                   
    df = df[['X','Y']]
    ...
    #GENETIC ALGORITHM FOR TRUCK-NODES TSP
    data = pd.read_csv("trucks_new_2drones.csv")
    def load_csv():                                                                
        data = pd.read_csv('trucks_new_2drones.csv').values
    ...
    SL = SL1+SL2
#GA TO FIND SHORTEST ROUTE FOR TRUCK-ONLY MODE (or TSP-0D)
    numberofcustomer = n                                             
    data2 = pd.read_csv("export_distance_2drones.csv") 
    def load_csv():
        data = pd.read_csv('export_distance_2drones.csv').values
        x, y = list(data[:, 0]), list(data[:, 1])
        return x, y
    best2 = 600
    tracing_list2 = []
...
df = pd.DataFrame(result_final, columns = ['INSTANCES', 
                                           ...
                                           'NUMBER OF POINTS CANNOT BE SERVED BY TRUCK-TSP ONLY WITHIN 4 HOURS',
                                           'NUMBER OF DRONE-POINTS  BE REPLACED BY TRUCK'])
df.to_csv (r'2drones-GeneticAlgorithm.csv', index = False, header=True)    
