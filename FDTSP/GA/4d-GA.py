...
epochs_ga = 10                                                            # epochs for whole GA 
chromosome = 50                                                           # the lenghth of chromosome in GA truck-TSP
terminal_condition = 20                                                   # convergence of minimizing distance
mutation_miltiplier = 5                                                   # the constant of mutation 
...
for i in range(0,iter_proposed_model):
    ...
    df.to_csv ('export_distance_4drones.csv', index = False, header=True)             
    df = pd.read_csv('export_distance_4drones.csv')                                   
    df = df[['X','Y']]
    ...
    pd.DataFrame(trucks_new).to_csv("trucks_new_4drones.csv", index=False)                       
    #GENETIC ALGORITHM FINDS TRUCK-NODES TSP
    data = pd.read_csv("trucks_new_4drones.csv")                
    def load_csv():                                                                
        data = pd.read_csv('trucks_new_4drones.csv').values
...
# GA FINDS SHORTEST ROUTE FOR TRUCK-ONLY MODE (or TSP-0D)
    ...
    data2 = pd.read_csv("export_distance_4drones.csv") 
    def load_csv():
        data = pd.read_csv('export_distance_4drones.csv').values
        x, y = list(data[:, 0]), list(data[:, 1])
        return x, y
    best2 = 600
    tracing_list2 = []
    ...
    #Distances between truck nodes and total time for the entired TSP
    distance_trucks2 = []
...
df = pd.DataFrame(result_final, columns = ['INSTANCES', 
                                           ...
                                           'NUMBER OF POINTS CANNOT BE SERVED BY TRUCK-TSP ONLY WITHIN 4 HOURS',
                                           'NUMBER OF DRONE-POINTS  BE REPLACED BY TRUCK'])
df.to_csv (r'4drones-GeneticAlgorithm.csv', index = False, header=True)
