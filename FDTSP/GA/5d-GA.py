...
for i in range(0,iter_proposed_model):
    ...
    df.to_csv ('export_distance_5drones.csv', index = False, header=True)            
    df = pd.read_csv('export_distance_5drones.csv')                                   
    df = df[['X','Y']]
    ...
    pd.DataFrame(trucks_new).to_csv("trucks_new_5drones.csv", index=False)                                                      
    #GENETIC ALGORITHM FINDS TRUCK-NODES TSP
    data = pd.read_csv("trucks_new_5drones.csv") 
    def load_csv():                                                                # TSP by GA begins
        data = pd.read_csv('trucks_new_5drones.csv').values
...
#GA FINDS SHORTEST ROUTE FOR TRUCK-ONLY MODE (or TSP-0D)
    ...
    data2 = pd.read_csv("export_distance_5drones.csv") 
    def load_csv():
        data = pd.read_csv('export_distance_5drones.csv').values
        x, y = list(data[:, 0]), list(data[:, 1])
        return x, y
    best2 = 600
    tracing_list2 = []
...
df = pd.DataFrame(result_final, columns = ['INSTANCES', 
                                           ...
                                           'NUMBER OF POINTS CANNOT BE SERVED BY TRUCK-TSP ONLY WITHIN 4 HOURS',
                                           'NUMBER OF DRONE-POINTS  BE REPLACED BY TRUCK'])
df.to_csv (r'5drones-GeneticAlgorithm.csv', index = False, header=True)
