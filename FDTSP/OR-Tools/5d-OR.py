...
def create_data_model():    
    data = {}
    data['distance_matrix'] = maps_centroids_2 #maps_matrix
    data['num_vehicles'] = 1
    data['depot'] = 0
    return data
...
for i in range(0,iter_proposed_model):
    ...
    for i, idx in enumerate(noLaunched_indices_d4):
        list_trucks.insert(i, list_dr4_new[idx + k])
        list_dr1_new.insert(i, [])
        list_dr2_new.insert(i, [])
        list_dr3_new.insert(i, [])
        list_dr4_new.insert(i, [])
    ...
    for i, idx in enumerate(noLaunched_indices_d5):
        list_trucks.insert(i, list_dr5_new[idx + k])
        list_dr1_new.insert(i, [])
        list_dr2_new.insert(i, [])
        list_dr3_new.insert(i, [])
        list_dr4_new.insert(i, [])
    ...
    for i, idx in enumerate(noLaunched_indices_d3):
        ...
        idx +=1
    for i, idx in enumerate(noLaunched_indices_d4):
        list_dr4_new[idx+count4] = []
        idx +=1
    for i, idx in enumerate(noLaunched_indices_d5):
        list_dr5_new[idx+count5] = []
        idx +=1
...
############################## COMBINATION BETWEEN TRUCK AND DRONES BEGIN ###########################
    ...
    d3_tr = []
    for i in range(len(routes_truck[0])):
        if list_dr3_new[routes_truck[0][i]]!=[]:
            tempo3 = math.sqrt((list_trucks_new[routes_truck[0][i]][0]-list_dr3_new[routes_truck[0][i]][0])**2+(list_trucks_new[routes_truck[0][i]][1]-list_dr3_new[routes_truck[0][i]][1])**2)
            d3_tr.append(tempo3)
        else:
            d3_tr.append(0)
    d4_tr = []
    for i in range(len(routes_truck[0])):
        if list_dr4_new[routes_truck[0][i]]!=[]:
            tempo4 = math.sqrt((list_trucks_new[routes_truck[0][i]][0]-list_dr4_new[routes_truck[0][i]][0])**2+(list_trucks_new[routes_truck[0][i]][1]-list_dr4_new[routes_truck[0][i]][1])**2)
            d4_tr.append(tempo4)
        else:
            d4_tr.append(0)
    d5_tr = []
    for i in range(len(routes_truck[0])):
        if list_dr5_new[routes_truck[0][i]]!=[]:
            tempo5 = math.sqrt((list_trucks_new[routes_truck[0][i]][0]-list_dr5_new[routes_truck[0][i]][0])**2+(list_trucks_new[routes_truck[0][i]][1]-list_dr5_new[routes_truck[0][i]][1])**2)
            d5_tr.append(tempo5)
        else:
            d5_tr.append(0)
    ...
    for i in range(len(routes_truck[0])-1):
        if list_dr3_new[routes_truck[0][i]]!=[]:
            tempo13 = math.sqrt((list_dr3_new[routes_truck[0][i]][0]-list_trucks_new[routes_truck[0][i+1]][0])**2+(list_dr3_new[routes_truck[0][i]][1]-list_trucks_new[routes_truck[0][i+1]][1])**2)
            d3_ntr.append(tempo13) 
        else:
            d3_ntr.append(0)
    for i in range(len(routes_truck[0])-1,len(routes_truck[0])):
        if list_dr3_new[routes_truck[0][i]]!=[]:
            tempo103 = math.sqrt((list_dr3_new[routes_truck[0][i]][0]-list_trucks_new[routes_truck[0][0]][0])**2+(list_dr3_new[routes_truck[0][i]][1]-list_trucks_new[routes_truck[0][0]][1])**2)
    ...
    for i in range(len(routes_truck[0])-1):
        if list_dr4_new[routes_truck[0][i]]!=[]:
            tempo14 = math.sqrt((list_dr4_new[routes_truck[0][i]][0]-list_trucks_new[routes_truck[0][i+1]][0])**2+(list_dr4_new[routes_truck[0][i]][1]-list_trucks_new[routes_truck[0][i+1]][1])**2)
            d4_ntr.append(tempo14) 
        else:
            d4_ntr.append(0)
    for i in range(len(routes_truck[0])-1,len(routes_truck[0])):
        if list_dr4_new[routes_truck[0][i]]!=[]:
            tempo104 = math.sqrt((list_dr4_new[routes_truck[0][i]][0]-list_trucks_new[routes_truck[0][0]][0])**2+(list_dr4_new[routes_truck[0][i]][1]-list_trucks_new[routes_truck[0][0]][1])**2)
    ...
    for i in range(len(routes_truck[0])-1):
        if list_dr5_new[routes_truck[0][i]]!=[]:
            tempo15 = math.sqrt((list_dr5_new[routes_truck[0][i]][0]-list_trucks_new[routes_truck[0][i+1]][0])**2+(list_dr5_new[routes_truck[0][i]][1]-list_trucks_new[routes_truck[0][i+1]][1])**2)
            d5_ntr.append(tempo15) 
        else:
            d5_ntr.append(0)
    for i in range(len(routes_truck[0])-1,len(routes_truck[0])):
        if list_dr5_new[routes_truck[0][i]]!=[]:
            tempo105 = math.sqrt((list_dr5_new[routes_truck[0][i]][0]-list_trucks_new[routes_truck[0][0]][0])**2+(list_dr5_new[routes_truck[0][i]][1]-list_trucks_new[routes_truck[0][0]][1])**2)
    ...
    for k in range(len(d1_tr)):
        if d1_travel[k] > endurance2*speed_drone and 2*d1_tr[k] < endurance2*speed_drone:
            ...
            d1_ntr_new[k] = 'NO LAUNCHED 2' # REPALCED NODES 1ST DRONE (IF)
        else:
            d1_tr_new[k] = d1_tr[k]
    ...
    for k in range(len(d2_tr)):
        if d2_travel[k] > endurance2*speed_drone and 2*d2_tr[k] < endurance2*speed_drone:
            ...
            d2_ntr_new[k] = 'NO LAUNCHED 2' # REPALCED NODES 2ND DRONE (IF)
        else:
            d2_tr_new[k] = d2_tr[k]
    ...
    for k in range(len(d3_tr)):
        if d3_travel[k] > endurance2*speed_drone and 2*d3_tr[k] < endurance2*speed_drone:
            ...
            d3_ntr_new[k] = 'NO LAUNCHED 2' # REPALCED NODES 3RD DRONE (IF)
        else:
            d3_tr_new[k] = d3_tr[k]
    ...
    for k in range(len(d4_tr)):
        if d4_travel[k] > endurance2*speed_drone and 2*d4_tr[k] < endurance2*speed_drone:
            d4_tr_new[k] = 'COMEBACK'
            d4_ntr_new[k] = 'COMEBACK' #RETURNED NODES 4TH DRONE
        elif d4_travel[k] > endurance2*speed_drone and 2*d4_tr[k] > endurance2*speed_drone:
            d4_tr_new[k] = 'NO LAUNCHED 2'
            d4_ntr_new[k] = 'NO LAUNCHED 2' # REPALCED NODES 4TH DRONE (IF)
        else:
            d4_tr_new[k] = d4_tr[k]
    ...
    for k in range(len(d5_tr)):
        if d5_travel[k] > endurance2*speed_drone and 2*d5_tr[k] < endurance2*speed_drone:
            d5_tr_new[k] = 'COMEBACK'
            d5_ntr_new[k] = 'COMEBACK' #RETURNED NODES 5TH DRONE
        elif d5_travel[k] > endurance2*speed_drone and 2*d5_tr[k] > endurance2*speed_drone:
            d5_tr_new[k] = 'NO LAUNCHED 2'
            d5_ntr_new[k] = 'NO LAUNCHED 2' # REPALCED NODES 5TH DRONE (IF)
        else:
            d5_tr_new[k] = d5_tr[k]
    ...
    for k in range(len(d2_tr_new)):
        if d2_tr_new[k] =='NO LAUNCHED 2' or d2_tr_new[k]== 0:
            ...
            service_d2[k] = 0.5 #0.5 MINUTE
    for k in range(len(d3_tr_new)):
        if d3_tr_new[k] =='NO LAUNCHED 2' or d3_tr_new[k]== 0:
            service_d3[k] = 0
        else:
            service_d3[k] = 0.5 #0.5 MINUTE
    for k in range(len(d4_tr_new)):
        if d4_tr_new[k] =='NO LAUNCHED 2' or d4_tr_new[k]== 0:
            service_d4[k] = 0
        else:
            service_d4[k] = 0.5 #0.5 MINUTE
    for k in range(len(d5_tr_new)):
        if d5_tr_new[k] =='NO LAUNCHED 2' or d5_tr_new[k]== 0:
            service_d5[k] = 0
        else:
            service_d5[k] = 0.5 #0.5 MINUTE
    ...
    within_d5 = [sum(v) for v in zip(travel_d4_tr,setup_d4,service_d4)]
    ...
    zipped2 = zip(list_trucks_new, list_dr1_new,list_dr2_new,list_dr3_new,list_dr4_new,list_dr5_new)
    ...
    no_service_by_dr4 = d4_tr_new.count('NO LAUNCHED 2')
    no_service_by_dr5 = d5_tr_new.count('NO LAUNCHED 2')
    SL2 = no_service_by_dr1 + no_service_by_dr2 + no_service_by_dr3 + no_service_by_dr4 + no_service_by_dr5
    SL = SL1+SL2    
...
# ############################## PERFORMING EVALUATION #############################################    
    ...
    count_deli4 = sum(map(lambda x : x != 'NO LAUNCHED 2' and x !=0, d4_tr_new))
    count_deli5 = sum(map(lambda x : x != 'NO LAUNCHED 2' and x !=0, d5_tr_new))
    count_deli=count_deli1+count_deli2+count_deli3+count_deli4+count_deli5
    dd = count_deli/n
    ...
    count_drone4_used_in_fact = sum([1 if x!=0 else 0 for x in d4_tr_new])
    count_drone5_used_in_fact = sum([1 if x!=0 else 0 for x in d5_tr_new])
    count_drones_used_in_fact = count_drone1_used_in_fact + count_drone2_used_in_fact + count_drone3_used_in_fact+count_drone4_used_in_fact+count_drone5_used_in_fact
    ...
    for j in d4_tr_test_note:
        count_dr4 = d4_tr_test_note.count('NO LAUNCHED')
    for j in d5_tr_test_note:
        count_dr5 = d5_tr_test_note.count('NO LAUNCHED')
    count_total = count_dr1+count_dr2+count_dr3+count_dr4+count_dr5
############################## RESULTS RECORDING #############################################    
    headers = ['INSTANCES', 
    ...
    for element in headers:
        if element not in result_final:
            ...
            if element == 'INSTANCES':
                result_final[element].append('1010_80_60_40')
...
df = pd.DataFrame(result_final, columns = ['INSTANCES', 
                                           ...
                                           'NUMBER OF POINTS CANNOT BE SERVED BY TRUCK-TSP ONLY WITHIN 4 HOURS',
                                           'NUMBER OF DRONE-POINTS  BE REPLACED BY TRUCK'])
df.to_csv (r'5DRONES-Ortool.csv', index = False, header=True)
