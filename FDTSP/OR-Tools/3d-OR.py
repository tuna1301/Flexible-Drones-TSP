...
for i in range(0,iter_proposed_model):
    ...
    for k in range(len(d3_tr_test)):
        if d3_tr_test[k] > (endurance1*speed_drone)/2:
            d3_tr_test_note[k] = 'NO LAUNCHED'
        else:
            d3_tr_test_note[k] = d3_tr_test[k]
    # REPLACING DRONES NODES BY TRUCK NODES
...
############################## COMBINATION BETWEEN TRUCK AND DRONES BEGIN ###########################
    ...
    for k in range(len(d2_tr)):
        if d2_travel[k] > endurance1*speed_drone and 2*d2_tr[k] < endurance1*speed_drone:
            ...
            d2_ntr_new[k] = 'NO LAUNCHED 2' # REPLACED NODES
        else:
            d2_tr_new[k] = d2_tr[k]
    ...
    for k in range(len(d3_tr)):
        if d3_travel[k] > endurance1*speed_drone and 2*d3_tr[k] < endurance1*speed_drone:
            d3_tr_new[k] = 'COMEBACK'
            d3_ntr_new[k] = 'COMEBACK' #RETURNED NODES
        elif d3_travel[k] > endurance1*speed_drone and 2*d3_tr[k] > endurance1*speed_drone:
            d3_tr_new[k] = 'NO LAUNCHED 2'
            d3_ntr_new[k] = 'NO LAUNCHED 2' # REPLACED NODES
        else:
            d3_tr_new[k] = d3_tr[k]
...
# ############################## PERFORMING EVALUATION #############################################    
    ...
    for j in d1_tr_test_note:
        count_dr1 = d1_tr_test_note.count('NO LAUNCHED')   # drone1 nodes replaced by trucks IF EXISTED
    ...
    for element in headers:
        if element not in result_final:
            ...
            if element == 'INSTANCES':
                result_final[element].append('1010_80_30_60')
...
df = pd.DataFrame(result_final, columns = ['INSTANCES', 
                                           ...
                                           'NUMBER OF POINTS CANNOT BE SERVED BY TRUCK-TSP ONLY WITHIN 4 HOURS',
                                           'NUMBER OF DRONE-POINTS  BE REPLACED BY TRUCK'])
df.to_csv (r'3DRONES-result-Ortool.csv', index = False, header=True)
